...
def team_summary(ball_df,match_df, selected_team, opponent_team):
    ...
    if total_matches>0:
        players = temp_df['Batter']._append(temp_df['Bowler'])
...
def team_top_scorers(df, selected_team, opponent_team):
    ...
    df = temp_df[['Batter','Batter Runs']].groupby(by='Batter')['Batter Runs'].sum().reset_index().sort_values('Batter Runs', ascending=False)
    df = df.reset_index()
    df = df.rename(columns={'Batter':'Player','Batter Runs':'Runs'})
...
def team_top_fours(df, selected_team, opponent_team):
    ...
    df = df[['Batter','Boundry']].groupby(by='Batter')['Boundry'].count().reset_index().sort_values('Boundry', ascending=False)
    df.reset_index(inplace=True)
    df = df.rename(columns={'Batter':'Player','Boundry':'Fours'})
    df = df.drop(columns=['index']).head(10)
    return df
...
def team_top_sixes(df, selected_team, opponent_team):
    ...
    df = df[['Batter','Boundry']].groupby(by='Batter')['Boundry'].count().reset_index().sort_values('Boundry', ascending=False)
    df.reset_index(inplace=True)
    df = df.rename(columns={'Batter':'Player','Boundry':'Sixes'})
    df = df.drop(columns=['index']).head(10)
    return df
...
def team_most_balls(df, selected_team, opponent_team):
    ...
    df = temp_df[['Batter','Ball']].groupby(by='Batter')['Ball'].count().reset_index().sort_values('Ball', ascending=False)
    df.reset_index(inplace=True)
    df = df.rename(columns={'Batter':'Player','Ball':'Balls Played'})
    df = df.drop(columns=['index']).head(10)
    return df
...
def best_strikerate(df, selected_team, opponent_team):
    ...
    df1 = temp_df[['Batter','Ball']].groupby(by='Batter')['Ball'].count().reset_index()
    df1 = df1.rename(columns={'Batter':'Player','Ball':'Balls Played'})

    df2 = temp_df[['Batter','Batter Runs']].groupby(by='Batter')['Batter Runs'].sum().reset_index()
    df2 = df2.rename(columns={'Batter':'Player','Batter Runs':'Runs'})
...
def yearwise_summary(new_ball_df, match_df, selected_year, selected_team):
    ...
    if total_matches>0:
        players = temp_df1['Batter']._append(temp_df1['Bowler'])
        total_players = players.nunique()
